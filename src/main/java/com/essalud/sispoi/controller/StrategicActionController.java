...
public class StrategicActionController {
    ...
    @PostMapping
    public ResponseEntity<Void> save(@Valid @RequestBody StrategicActionDTO dto) {
        StrategicAction p = service.save(mapper.map(dto, StrategicAction.class));
        URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(p.getIdStrategicAction()).toUri();
