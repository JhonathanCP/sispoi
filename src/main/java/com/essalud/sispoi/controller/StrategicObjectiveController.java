...
public class StrategicObjectiveController {
    ...
    @PostMapping
    public ResponseEntity<Void> save(@Valid @RequestBody StrategicObjectiveDTO dto) {
        StrategicObjective p = service.save(mapper.map(dto, StrategicObjective.class));
        URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(p.getIdStrategicObjective()).toUri();
