...
public class PriorityController {
    ...
    @PostMapping
    public ResponseEntity<Void> save(@Valid @RequestBody PriorityDTO dto) {
        Priority p = service.save(mapper.map(dto, Priority.class));
        URI location = ServletUriComponentsBuilder.fromCurrentRequest().path("/{id}").buildAndExpand(p.getIdPriority()).toUri();
